...
const Dashboard = () => {
  ...
  const { totalIncome, totalExpenses, balance, recentTx } = useMemo(() => {
    const inc = incomes ?? [];
    const exp = expenses ?? [];
    const totalIncome = inc.reduce((s, i) => s + i.amount, 0);
    const totalExpenses = exp.reduce((s, e) => s + e.amount, 0);

    const incomeRows: Transaction[] = inc.map((i) => ({
      id: i.id,
      label: i.source,
      amount: i.amount,
      date: i.date,
    ...
    const expenseRows: Transaction[] = exp.map((e) => ({
      id: e.id,
    ...
    return { totalIncome, totalExpenses, balance: totalIncome - totalExpenses, recentTx };
  }, [incomes, expenses]);
