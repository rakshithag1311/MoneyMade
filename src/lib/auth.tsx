...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Listen for auth changes — skip ensureProfile on token refreshes for the same user
    let lastUserId: string | null = null;
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      setSession(session);
      setUser(session?.user ?? null);

      // Only run profile setup on actual sign-in / sign-up, not on every token refresh
      const isNewUser = session?.user?.id !== lastUserId;
